...
export const placeBid = async (req, res) => {
  try {
    ...
    const bidStartTimeIST = moment(product.bidStartTime).tz("Asia/Kolkata");
    const bidEndTimeIST = moment(product.bidEndTime).tz("Asia/Kolkata");

    // Log product's bid start and end times in IST
    console.log("Bid Start Time (IST):", bidStartTimeIST.format());
    console.log("Bid End Time (IST):", bidEndTimeIST.format());

    // Check if current time is within bidding time
    if (
      currentTimeIST.isBefore(bidStartTimeIST) ||
      currentTimeIST.isAfter(bidEndTimeIST)
    ) {
      console.log("Bidding time is not valid");
      return res.status(400).json({ message: "Bidding time is not valid" });
    }
    ...
    // Check if bidding time has ended
    if (currentTimeIST.isAfter(bidEndTimeIST)) {
      // Update product bidding status to indicate bidding has ended
      product.biddingStatus = "Bidding Ended";
      await product.save();
